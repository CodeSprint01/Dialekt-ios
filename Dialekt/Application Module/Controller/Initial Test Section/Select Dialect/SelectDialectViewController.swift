//
//  SelectDialectViewController.swift
//  Dialekt
//
...
class SelectDialectViewController: UIViewController {
    ...
    var allCitiesData = [ParticularLanguageModel]()
    var total_pages = 1
    var current_page = 1
    var isFromSignUp = false
    var isFromSocialLogin = false
    var isFromChooseLanguage = false
    var SelectedCity = "" 
    ...
    override func viewDidLoad() {
        ...
        apiCallForDailetList(search: "", Page: 1 , true)
    }
    
    //MARK:- TEXT IS CHANGING
    @objc func TextIsChanging(_ tf : UITextField) {
        apiCallForDailetList(search: tf.text?.trimmed() ?? "", Page: 1)
    }
...
extension SelectDialectViewController : UITextFieldDelegate {
          ...
          if scrollView == CollectionView{
              if (scrollView.contentOffset.y == /*0*/ maximumOffset) {
                  ...
                  if remainingPagesCount < 1{
                    //  UniversalMethod.universalManager.alertMessage("No Video left", self.navigationController!)
                  }else{
                    let pageNo = current_page + 1
                    apiCallForDailetList(search: tfSearch.text?.trimmed() ?? "", Page: pageNo)
                  }
              }
          }
      }
