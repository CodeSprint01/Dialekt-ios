...
class TestProgressVC: UIViewController {
    ...
    var gameId = 0 //this variable will be used to Level Game
    var isFromGameLevel = false // true when it is From Game level
    var CorrectAnswerCount = 0 //Used for Initial Test Only
    ...
    override func viewDidLoad() {
        ...
        if isFromGameLevel {
            apiCallForLevelQuestionList()
        } else {
            apiCallForQuestionList()
        }
        //MARK:- OBSERVER ADD TO GO TO SIGNUP SCREEN AND [IT POST IN TestPerformanceVC].
